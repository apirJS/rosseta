export function buildGroqTranslationPrompt(targetLanguage: string): string {
...
3) \`originalText.contents[i] = { text, languageBcp47Code, language, romanization? }\`
   ...
   - \`languageBcp47Code\`: BCP-47 with region (e.g., \`en-US\`, \`id-ID\`, \`ja-JP\`). If region uncertain, pick a common default. Special codes: \`"number"\` for purely numeric segments, \`"symbol"\` for symbol/punctuation/emoji-only segments, \`"unknown"\` if the language truly cannot be identified. **NEVER use \`"und"\` or \`"Undetermined"\`.**
